...
The committed CI manifest is `scripts/golden_manifest.ci.tsv`. Local manifests
can still use `scripts/golden_manifest.tsv` (gitignored).
...
CI requires `scripts/golden_manifest.ci.tsv`; a missing manifest is a failure.
The committed baseline was generated by MobilityData's canonical validator
v8.0.1. Its CLI asset SHA-256 is
...
fixture, or golden tooling changes. `workflow_dispatch` accepts a custom
manifest path. JSON coordinates are rounded to 12 decimal places to ignore
equivalent Java/Rust floating-point renderings while preserving useful parity.
